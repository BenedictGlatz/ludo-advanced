...
| NFR-11 † | A player action produces visible feedback within 100 ms. No frame-rate target is set — the game is turn-based. | Measured on the reference machine during the buffer-sprint playtest. | S | NFG-08 |
...
2. **Should-haves next**, keeping FR-35 (rules screen) and NFR-12 (colour-independent players) for
   last — both are preconditions for the buffer-sprint playtest with people who get no instructions,
   which is where the usability evidence for the report comes from.
