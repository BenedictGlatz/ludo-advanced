...
| File | Contains |
| --- | --- |
| `design/handoff/06-spec-pawn-mark.md` | The five-section spec: files delivered, D48 to D50 answered with reasons and rejected alternatives, tokens added if any, states covered against § 3, what is still open |
...
| `design/handoff/handoff-06/` | Optional. A mockup shell if you need one to look at it, in its own folder with a `README.md` saying it is not production code. It is deleted after the review, the same as `handoff-04/` |
