...
- **2026-08-06**: `CLAUDE.md`, `README.md` and `CHANGELOG.md` written; stack fixed to
  ...
  Sprint 0.
- **2026-08-06**: Documentation notes established under `docs/documentation/`: steering index,
...
- **2026-08-09**: SMART analysis written for issue #9: one overall project goal plus four sub-goals,
  ...
  `docs/project-management/smart-analysis.md`; facts in Ch. 01. Sprint 0.
- **2026-08-09**: Feasibility study written for issue #12, assessing the chosen 2D option across
  ...
  `docs/project-management/feasibility-study.md`; facts in Ch. 03 and Ch. 10. Sprint 0.
- **2026-08-10**: Risk register in `risk-analysis.md` expanded from 3 to 16 risks, all traced to
...
- **2026-08-29**: Design handoff loop established for issue #3: `design/` with a README, the brief
...
- **2026-08-30**: Design handoff 01 landed for issue #3. Five stylesheets in `src/ui/styles/` and the
  spec in `design/handoff/`. The five landing checks from the sprint plan were run: sixteen
...
- **2026-09-06**: The *Design and UI* convention in `CLAUDE.md` was changed at the Product Owner's
  ...
  small; Claude Design keeps new designs and the design system. `design/README.md` gained a
...
- **2026-09-06, night**: Skill card animations planned. `Skill-Card-Animations-Brief.md` is design
  ...
  at `design/handoff/18-brief-skill-card-animations.md` when the round is actually run. No code
  changed. Sprint 3.
...
- **Source:** [Meeting Notes 20260806](../project-management/meeting-notes/2026-08-06.md),
  [one-pager.md](../project-management/one-pager.md).
...
- **Chosen:** `docs/ai-prompts/` added to `.gitignore`; the two existing tracked files
...
- **Chosen:** a `docs/documentation/` directory of fact-only chapter notes, updated in the same
...
- **Rejected:** *treating [github-project.md](../project-management/github-project.md) as
...
- **Consequence:** the risk row *Board layout & win conditions underspecified* is re-rated from
  priority 4 to 3 in [risk-analysis.md](../project-management/risk-analysis.md). Likelihood
...
- **Chosen:** the rewritten [one-pager.md](../project-management/one-pager.md) keeps the base
...
- **Chosen:** the 300-line limit in [CLAUDE.md](../../CLAUDE.md) is read as binding on source, tests
  and config, and not on documents under `docs/project-management/`. The game design document is
...
- **Why this is a reading and not an exception:** the limit exists so that a unit of code stays small
  ...
  `docs/documentation/` carries the same exemption explicitly.
...
- **Chosen:** the role table of [one-pager.md](../project-management/one-pager.md) holds. Fabian
  ...
  [github-project.md](../project-management/github-project.md) is superseded and kept in place with
  a note, not deleted.
...
- **Chosen:** `.prettierignore` excludes every `*.md` file and the whole of `docs/`. `npm run
...
- **Chosen:** every square, every pawn and every start slot is a real DOM element, laid out by CSS
  ...
  [01-brief-foundations-and-board.md](../../design/handoff/01-brief-foundations-and-board.md)
  section 2.
...
- **Chosen:** a top-level `design/` folder, committed to git, holding a numbered brief and spec per
...
- **Why the CSS goes straight into `src/ui/styles/` and not into `design/`:** the design system
...
- **Why the reasoning stays in `design/` instead:** "what does it look like" belongs with the code
...
- **Rejected: keeping the handoff in the issue thread on GitHub**, which is where a design
  ...
  artefact.
- **Rejected: putting the design documents under `docs/documentation/`** with the chapter notes.
...
- **Consequence:** `design/` is a third documentation directory. The 300-line limit is read as not
...
### 2026-08-30: ESLint ignores `design/`

- **Chosen:** `design/**` is in the ignore list in `eslint.config.js`.
...
- **Why this is safe:** nothing under `design/` is built or shipped. `design/README.md` is
...
- **Rejected: a build step.** It would make every `npm run build` depend on a file in `design/`,
...
- **Reported to the Product Owner** rather than corrected in the delivered spec, since
  `design/handoff/` is the design side's document.
- → Ch. 04
...
- **Filed back to design:** the delivered handoff 16 spec puts the layer on the strip, so
  `design/handoff/00-open-requests.md` records that the code no longer follows it.
...
- **The lobby was built without a design handoff**, which `CLAUDE.md` gives a new screen to. Four
  ...
  [00-open-requests.md](../../design/handoff/00-open-requests.md), and `lobby.css` says in its
...
### 2026-09-11: Every file and folder name is lowercase kebab-case, and the two top folders become `docs/` and `design/`

- **Chosen:** one naming rule for the whole repository, written into `CLAUDE.md` § File naming.
  `src/`, `tests/` and `scripts/` already followed it; `00-Meta/` and `01-Design/` did not. They mixed
  Title-Case (`Project-Management/`, `Handoff/`), spaces (`Meeting Notes/`,
  `Card artwork design planning/`), a date without dashes (`20260806.md`) and numbers on four of twenty
  planning documents. Every path that broke the rule was renamed, and every reference to it rewritten.
- **Why lowercase kebab-case:** it is the web convention, and it is the only one that is safe everywhere
  this project runs. GitHub Pages serves from Linux, where `Handoff/` and `handoff/` are two different
  folders, while Windows treats them as one; a link that works on the developer's machine can 404 online.
  Spaces become `%20` in every Markdown link, which is why five links to the meeting notes carried
  `Meeting%20Notes`.
- **Rejected:** *`00-meta/` and `01-design/`, keeping the numbers.* The numbers only sorted the two
  folders above `src/`, and `docs/` is the name every web project and GitHub itself uses for
  documentation.
- **Rejected:** *numbering every planning document.* The four numbered ones (`00-One-Pager.md` to
  `03-Risk-Analysis.md`) have no reading order the other sixteen lack, so a number would claim an order
  that does not exist. Numbers stay where order is real: the thirteen report chapters, the handoff rounds
  and the figures.
- **Rejected:** *renaming the inside of the Claude Design export.* Its `_ds/` bundle and `.dc.html`
  canvases are that tool's own output, and renaming them risks the canvas no longer opening or syncing.
  Only the outer folder moved, to `design/handoff/card-art-canvas/`.
- **Not rewritten on purpose:** the prompt log (existing entries are never rewritten, so old prompts keep
  the old paths verbatim), the generated `13-ai-index.md` apart from its `Source:` line, and GitHub
  issues and pull requests, which keep links to the old paths.
- **What it costs:** the finished report (`.docx`) cites repository paths as its digital appendix, and
  those have to be corrected by hand. Every contributor also has to move their local, gitignored
  `00-Meta/AI-Prompts/<github-username>/` folder to `docs/ai-prompts/<github-username>/` after pulling,
...
- **2026-08-31: Landing one design spec turned up four defects, and only one of them was in the
  spec.** The five-item entrance check in `design/README.md` says not to merge a spec unread. On a
