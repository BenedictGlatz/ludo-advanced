...
- **`.prettierignore` excludes every markdown file and the whole of `docs/`.** This is deliberate
...
**`design/**` is now ignored by ESLint, and the reason matters more than the line.** Claude Design
...
errors, none of them in project code**. Nothing under `design/` is built or shipped:
`design/README.md` is explicit that the CSS lands in `src/ui/styles/` instead.
...
- **The cheap fix that needs no dependency** is a line in the design brief template saying the size
  ...
  `design/README.md` and it costs nothing.
...
### One naming rule for every file and folder: 2026-09-11

- **The rule:** lowercase kebab-case, no spaces, numbers only where order means something, ISO dates,
  `README.md` as a folder's index. Written into `CLAUDE.md` § File naming. Decision and rejected
...
- **Tooling that named the old paths and was updated:** `.gitignore` (the prompt log is now
  ...
  fixed by hand.
- **How the result was checked:** a throwaway script resolved every relative Markdown link in the
  repository before and after the rename. Both runs found the same broken links, all in two design
  handoff files (`12-spec-main-menu.md`, `18-brief-skill-card-animations.md`) and all broken before the
  rename; the rename added none. Those links are left for a separate fix.
