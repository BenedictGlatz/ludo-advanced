...
`design/handoff/card-art-canvas/` has spaces in its name, so every file under it was
...
- **The pathspec narrows to `src/`, `tests/`, `scripts/` and the config files**, and that is a
  ...
  JavaScript files in the repository are `design/handoff/card-art-canvas/support.js` and
