...
1.263. `design/assets/board-greyscale.png` shows what that looks like: three of the four yards are
the same grey.
...
| `npm run format` | Prettier is not one of the five named gates, and it ignores `*.md` and `docs/`, so a `--check` gate would police a fraction of the repository. Adding it is a new rule and belongs in its own issue. | On demand, by whoever edits code |
