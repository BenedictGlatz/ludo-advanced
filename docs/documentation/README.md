...
| 04 | [04-frontend-building-blocks.md](notes/04-frontend-building-blocks.md) | `src/ui/`, `src/i18n/`: components, rendering, event binding, localisation | ready: every screen built, seven design handoffs landed and recorded, no placeholder stylesheet left; two briefs are out (D61 to D64) |
...
notes. They live in `docs/project-management/` and are cited from the chapter that uses them:
...
- ~~**The two role tables contradict each other.**~~ **Resolved 2026-08-22, issue #15**, in section 3.1
  ...
  [one-pager.md](../project-management/one-pager.md) table holds (Fabian Gemming Product Owner,
  Lars Bolender and Benedict Glatz Scrum Members, no dedicated Scrum Master) and the unnamed Developer
  A/B/C table in [github-project.md](../project-management/github-project.md) is superseded as an
