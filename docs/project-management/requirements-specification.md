...
the Product Owner that morning. Issue #82 replaced it: a bot prices every card in its hand in the units
of a move and plays the best one when it is worth more than keeping it. The wording says *a value model*
rather than *plays cards*, because "plays cards" is satisfied by a bot that plays them at random, and
the difference between the two is the whole feature. Two cards, Oil Spill and The Purge, are never
played, and each refusal is a recorded finding with its reason in Ch. 06.
...
Sprint plan from [github-project.md](github-project.md), mapped to requirement blocks:
...
- **FR-37, the resource/energy system.** [github-project.md](github-project.md) plans it for
