...
**The mechanics reduce to data and pure functions, with one exception.** The Dice Card Pool is a draw
of three from a finite set of D2–D20 cards, a selection, a random number in a range, and a return of
the three cards. The Skill Card Pool is a deck, a hand, and per-card effects applied to a game state:
true for `Action` cards, and true for `Reaction` cards once their resolution order is fixed, which is
...
**Open technical points, named rather than hidden.** The scope boundary in this study excludes
gameplay-concept questions such as the exact win condition or the energy/resource system: those are
rulebook and scope decisions, not engineering risk, and belong to epic planning rather than here. What
follows is engineering work the current documentation does not yet decide:

- **Reaction-card resolution has no timing model.** `Reaction` cards are playable in response to
  another player's action, which is an interrupt, not a plain state transition. Nothing on record
  fixes when a reaction window opens, in what order multiple reactions resolve, or how an interrupted
  action resumes afterwards. Modelling this as a small resolution stack in `core/` is a known, solved
  pattern, comparable to layered effect resolution in other card games, but it is real design work,
  not a pure function applied directly to state the way the Dice Card Pool is.
- **The DOM update strategy for `ui/` is undecided.** The stack is jQuery, without a virtual DOM.
  Nothing on record decides whether a state change triggers a full re-render of the affected board
  region or a targeted patch of the changed elements. Left undecided, the usual failure mode is event
  handlers rebound on every render without the old ones removed, and drift between the displayed board
  and the actual state.
- **Animation is not sequenced against turn state.** A pawn moving several squares, a capture, and a
  card draw all need visible animation, and that animation runs asynchronously while the turn state
  advances synchronously. Nothing on record decides how player input is locked out until an animation
  finishes, or how two animations triggered by the same action, such as a move and the capture it
  causes, run relative to each other.
- **The Dice Card Pool has no seeded RNG.** The ≥ 80 % coverage target in `core/` meets a system built
  on randomness. Without an injectable, seedable random source, dice draws cannot be tested
  deterministically, and coverage of the pool's branches would depend on which random values a test
  run happens to produce.
- **E2E tests will run against animated state.** Playwright specs asserting on a board position that
  only settles after a CSS transition or a jQuery `.animate()` call are a known source of flaky tests
  unless they wait on animation completion rather than a fixed delay. Worth naming now, before the
  first E2E spec is written.

None of these blocks delivery: each is a design decision to make early rather than a capability the
team lacks. The reaction-card resolution model and the RNG seeding decision are the two worth deciding
before the Skill Card Pool and Dice Card Pool are implemented, since retrofitting either into code
already written costs more than deciding first.

**Verdict: feasible.** The chosen stack is not merely adequate for this game. It is oversized for it,
and for a fixed deadline that is the correct direction to err in.
...
the toolchain, create the build, wire up tests and linting. That work is not in Sprint 1's planned
scope, and it is also the sprint that carries the largest epic (#36, four sub-issues). This is the
...
is 1½ weeks. Two readings are both live, and nothing in the repository decides between them. Sprint 3
may *be* the buffer under another name, in which case the polish, audio and menu scope has to move
earlier. Or the buffer was dropped, in which case there is no slack at all behind 2026-09-17.

**The planned sprint scope is wider than the goal.** Sprint 2 in
[github-project.md](github-project.md) lists multiplayer and the resource/energy system, and
neither is part of the `must have` set. Reconciling the sprint plan with the MoSCoW labels frees up
schedule rather than costing it. This is the one place in the study where the finding is favourable.
...
| --- | --- | --- |
| Technical | Feasible | Reaction-card resolution model, RNG seeding and the animation/turn-lock strategy decided before implementation starts |
...
a competence question: "can two of the three of us learn C# and still ship." After, it is a schedule
question: "can two implementers finish four epics in eight weeks." The Nutzwertanalyse scores that
