...
in [one-pager.md](one-pager.md), to give the player "more options than just rolling dice", and
...
| --- | --- | --- |
| **S** | It names the player count, the two card pools with their exact draw rule, and the win path. A build that renders a board but never reaches a victory does not meet it. | [one-pager.md](one-pager.md) |
...
| **T** | End of Sprint 1 on the board, matching the Sprint 1 scope in [github-project.md](github-project.md). |
...
| A runnable test and build setup | The ≥ 80 % coverage criterion. No `package.json` and no source code exist yet, so `npm run test:coverage` has never produced a reading, so the figure is a target and not a status. | Scrum members, with the first `src/` commit | Sprint 1 |
...
[documentation/README.md](../documentation/README.md). Two further contradictions come with the board dates and
